...
from .crud import guardar_nomina
...
@app.post("/calcular/")
def calcular_nomina_endpoint(data: DatosNomina, db: Session = Depends(get_db)):
    result = calcular_nomina(
        data.tipo_contrato,
        data.horas_diurnas,
        data.horas_nocturnas,
        data.horas_dominicales,
        data.valor_hora
    )
    nomina_db = guardar_nomina(db, result)
    return {"mensaje": "Nomina guardada", "resultado": result}
